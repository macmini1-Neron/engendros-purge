...
> **Accuracy reference:** see **`docs/superpowers/specs/t90m-reference.md`** (user-supplied) for the precise T-90M look + copy-paste model prompt. Silhouette priorities: **low / wide / squat** (width dominates height — NOT tall like Abrams/Leopard); proportions ~9.5 m gun-forward, hull ~6.86 m, width ~3.5 m, height ~2.3 m; **exactly 6 large road wheels per side** (key recognizer); **welded ANGULAR turret** (not a rounded cast dome); 125 mm smoothbore with segmented thermal sleeve + bore-evacuator bulge and **NO muzzle brake**; modular **Relikt ERA** tiles (front glacis + turret cheeks); side-skirt panel rows; **rear bustle + slat/cage armor**; **smoke-grenade tube clusters** on turret sides; busy roof (cupola, panoramic sight, antennas); rear engine-deck grilles.
