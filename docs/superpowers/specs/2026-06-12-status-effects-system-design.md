...
- **P3 — burn migration + co-op + FX polish (separate plan):** migrate **burn** into effects including its `mp.js` co-op paths (retire `pstate.burnT`/`_tickBurn`/`bf`); host-authoritative effect apply/tick + broadcast; enemy effect FX polish (radiation glow, richer «пух»). Also fold in two final-review follow-ups: (a) `player.reset()` should call `clearEffects()` rather than a raw `Map.clear()`, so any future side-ful `onClear` runs on game restart (benign today — `broken_leg.onClear` only clears `legBroken`, which `resetStats()` also does); (b) guard or document that console-applied effects only tick on `hostSim` — a co-op client's `/effect` self-apply shows a HUD chip that won't tick until host-auth effect sync lands.
