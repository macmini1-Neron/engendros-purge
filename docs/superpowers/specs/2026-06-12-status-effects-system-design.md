...
- **P3 — burn migration + co-op + FX polish (separate plan):** migrate **burn** into effects including its `mp.js` co-op paths (retire `pstate.burnT`/`_tickBurn`/`bf`); host-authoritative effect apply/tick + broadcast; enemy effect FX polish (radiation glow, richer «пух»). **Remaining review follow-up:** console-applied effects only tick on `hostSim` — a co-op client's `/effect` self-apply shows a HUD chip that won't tick until host-auth effect sync lands (this is the same host-authoritative work P3 already covers). *(The other follow-up — routing `reset()`/`applySplint` through `clearEffects`/`removeEffect` so `onClear` fires instead of a raw `Map.clear()` — was done in the review hardening pass, c38b2154.)*

> **P1+P2 status (2026-06-12): IMPLEMENTED on `feat/playable-demo`** — 10 commits (`8d1ce4b8`..`4fe93f6c`). 221/221 node tests green; final holistic review = merge-ready, no blockers. Browser boot-smoke verified the full chain: radiation −HP player (−0.7/tick) / +HP enemy (+1.2/tick), bleed = no enemy HP change, 0 console errors. Feel/visual acceptance (HUD countdown, visible slow/limp, «пух» puffs, Tab-complete) is the player's in-game playtest.
>
> **Review + hardening (2026-06-12, c38b2154):** PR #42 reviewed by a 5-agent panel (code / tests / silent-failures / comments / type-design) → unanimous **merge-ready, 0 blockers**. Applied the safe wins: `removeEffect()` API (closes the `applySplint`/`reset` encapsulation gap), NaN/non-positive-seconds guard, `def`-guards in `stepEffects`/`movementSlow`/`contactWeaken`, console honesty (count-on-success + a `targets:'player'` guard so `/effect @e broken_leg` no longer lies), `setSurvival` per-frame innerHTML cache, comment-accuracy fixes, and **+9 tests** (now **28/28** effects · **230/230** repo). Re-verified by browser boot-smoke (0 console errors).
