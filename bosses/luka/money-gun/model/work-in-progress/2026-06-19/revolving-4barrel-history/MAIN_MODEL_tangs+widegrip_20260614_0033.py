...
# CONSTANTS  (metres)
# =====================================================================
# Ornament toggle (owner 2026-06-13: strip all decoration, focus on the MODEL
# shape first — flip back to True later to re-enable grip filigree, butt acanthus
# and lock engraving). Override before exec via globals()["DECOR"]=True.
DECOR        = False
...
def boolean_diff(target, cutter):
    """Subtract cutter from target (applied) and remove the cutter object."""
    bpy.ops.object.select_all(action='DESELECT')
    bpy.context.view_layer.objects.active = target
    m = target.modifiers.new("bool", 'BOOLEAN'); m.operation = 'DIFFERENCE'; m.object = cutter
    try: m.solver = 'EXACT'
...
def build_barrel_cluster():
    ...
    for cy, cz in BARRELS:
        for bx in (BREECH_X + BARREL_L*OCTA_FRAC, BREECH_X + BARREL_L*(OCTA_FRAC+0.02)):
            ...
            extras.append(ring)
    # gold collar wrapping the BARREL/BREECH JOINT — a STATIC sleeve (owner: this is
    # a fixed part, it does NOT revolve with the barrels). Rounder section + softer
    # edges, sits behind the breech ring and hides the revolving mechanism.
    # NOTE: deliberately NOT appended to `extras` so it stays off the rotating
    # cluster; it is returned separately and parented to the static root in main().
    collar = add_cylinder("collar", r=RC+BARREL_R+0.0024, depth=0.020,
                          loc=(BREECH_X-0.0085, 0, 0), verts=28, rot=(0, radians(90), 0))
    add_bevel(collar, width=0.0024, segments=4); add_subsurf(collar, vp=1, rnd=2)
    # gold collar band AT the muzzle end (owner: put it right at the muzzle like the
    # photo) — front edge flush with the muzzle face, just behind the clover cap.
    muzcol = add_cylinder("muz_collar", r=RC+BARREL_R+0.0013, depth=0.010,
                          loc=(MUZZLE_X-0.005, 0, 0), verts=24, rot=(0, radians(90), 0))
    # drill the 4 barrel bores THROUGH the collar so the muzzle holes stay visible
    # (owner: 'proděrav tu objímku' — otherwise the band caps the barrels).
    for (cy, cz) in BARRELS:
        cutter = add_cylinder(f"muzcut_{cy}_{cz}", r=BORE_R+0.0012, depth=0.04,
                              loc=(MUZZLE_X-0.005, cy, cz), verts=20, rot=(0, radians(90), 0))
        boolean_diff(muzcol, cutter)
    # scallop the 4 DIAGONAL outer corners (the valleys between barrels) so the
    # collar outline becomes a clover/quatrefoil, not a plain circle (owner sketch).
    gdiag = 0.0142
    for (sy, sz) in [(gdiag, gdiag), (gdiag, -gdiag), (-gdiag, gdiag), (-gdiag, -gdiag)]:
        sc = add_cylinder(f"muzscallop_{sy:.4f}_{sz:.4f}", r=0.0078, depth=0.05,
                          loc=(MUZZLE_X-0.005, sy, sz), verts=20, rot=(0, radians(90), 0))
        boolean_diff(muzcol, sc)
    add_bevel(muzcol, width=0.0006, segments=2)
    ...
    breech_ring = add_torus("breech_ring", major=RC+BARREL_R+0.0014, minor=0.0016,
                            loc=(BREECH_X+0.004, 0, 0), rot=(0, radians(90), 0), mseg=32, miseg=16)
    extras.append(breech_ring)
    # fill the gaps between the barrels INSIDE the ring with a solid disc (breech
    # face) — only as thick as the ring (owner). Ring stays as a raised rim.
    breech_fill = add_cylinder("breech_fill", r=RC+BARREL_R+0.0014, depth=0.0030,
                               loc=(BREECH_X+0.004, 0, 0), verts=32, rot=(0, radians(90), 0))
    add_bevel(breech_fill, width=0.0006, segments=2)
    extras.append(breech_fill)
    # thin SMOOTH wheel/band nestled BETWEEN the collar and the breech ring
    # (owner img 12/13) — NOT toothed; a little smaller than the collar/ring.
    bwheel = add_cylinder("BreechWheel", r=0.0172, depth=0.0046,
                          loc=(-0.0795, 0, 0), verts=44, rot=(0, radians(90), 0))
    add_bevel(bwheel, width=0.0011, segments=3)
    extras.append(bwheel)

    # --- gold muzzle: ONE small ring at the END of each barrel (owner) ---
    # No center boss, no big rims/pins — just a thin ring around each bore mouth,
    # a couple of mm SMALLER than the barrel so it sits INSIDE the barrel edge.
    cap_parts = []
    capx = MUZZLE_X
    for cy, cz in BARRELS:
        rim = add_torus(f"muzrim_{cy}_{cz}", major=BARREL_R-0.0016, minor=0.0009,
                        loc=(capx, cy, cz), rot=(0, radians(90), 0), mseg=24, miseg=8)
        cap_parts.append(rim)
    # small ROUNDED studs at the 4 lobe tips (the nubs on the real Denix muzzle);
    # the top one doubles as the front sight (owner: 'muška'). Rounded, not pointed,
    # and sitting in the MIDDLE of the collar width (not on the front face).
    R_out = RC + BARREL_R + 0.0013
    studx = MUZZLE_X - 0.005                  # middle of the collar depth
    for (uy, uz) in [(0, 1), (0, -1), (1, 0), (-1, 0)]:
        ball = add_sphere(f"muzstud_{uy}_{uz}", r=0.0016,
                          loc=(studx, uy*(R_out+0.0004), uz*(R_out+0.0004)), segs=16, rings=10)
        cap_parts.append(ball)
        # thin COLLAR band around the ball's EQUATOR (exact half), same outer
        # diameter as the ball so it reads as a belt, not a proud ferrule (owner).
        rt = R_out + 0.0004                       # = ball centre -> ball's half/equator
        ring_rot = (0, 0, 0) if uy == 0 else (radians(90), 0, 0)
        ring = add_torus(f"muzbezel_{uy}_{uz}", major=0.00115, minor=0.00046,
                         loc=(studx, uy*rt, uz*rt), rot=ring_rot, mseg=20, miseg=8)
        cap_parts.append(ring)
    ...
    return cluster_all, bore_obj, collar
...
def build_lock():
    ...
    cy = LY + LSIDE*0.007           # cock proud of the breech side
    # --- COCK: re-traced S goose-neck (ref img 7) as a FLAT plate (ribbon) + a
    #     lathed baluster URN finial + jaws + thumb spur with a ball tip (owner
    #     rework). S centreline traced off image 7 as (x, z, half-width): wide
    #     tumbler base -> narrow throat -> back-bulge -> jaws. Muzzle = +X. ---
    # smooth flowing S via a bezier swept tube (auto handles), then FLATTEN in Y
    # to a plate. Centreline traced off image 7; muzzle = +X.
    neck_cl = [(-0.1085, 0.0, 0.0140), (-0.1068, 0.0, 0.0250), (-0.1135, 0.0, 0.0340),
               (-0.1206, 0.0, 0.0420), (-0.1140, 0.0, 0.0490), (-0.1112, 0.0, 0.0515)]
    neck_taper = [(-1, 1.5), (-0.5, 1.0), (0.1, 0.7), (0.6, 0.85), (1, 0.9)]
    neck = build_curve_tube("Cock", neck_cl, bevel=0.0044, taper=neck_taper)
    neck.scale = (1.0, 0.58, 1.0); neck.location.y = cy           # flatten to a plate
    cparts = [neck]
    # jaw block at the top of the neck (clamps the flint)
    jx, jz = -0.1112, 0.0512
    jaw = add_cube("cock_jaw", 1.0, loc=(jx, cy, jz)); jaw.scale = (0.0062, 0.0042, 0.0052)
    add_bevel(jaw, width=0.0008, segments=2); cparts.append(jaw)
    # vertical jaw screw, capped by the baluster urn finial
    cparts.append(add_cylinder("cock_jscrew", r=0.0016, depth=0.010,
                               loc=(jx, cy, jz+0.006), verts=12, rot=(0, 0, 0)))
    # lathe the URN finial about the vertical (z) axis through (jx, cy)
    urn_prof = [(0.0058, 0.0508), (0.0067, 0.0526), (0.0049, 0.0542), (0.0032, 0.0562),
                (0.0050, 0.0590), (0.0030, 0.0616), (0.0043, 0.0638), (0.0026, 0.0654),
                (0.0, 0.0664)]
    ubm = bmesh.new(); uns = 22; urings = []; uapex = None
    for (r, zz) in urn_prof:
        if r <= 0.0:
            uapex = ubm.verts.new((jx, cy, zz)); continue
        urings.append([ubm.verts.new((jx + r*cos(2*pi*j/uns), cy + r*sin(2*pi*j/uns), zz))
                       for j in range(uns)])
    for i in range(len(urings)-1):
        for j in range(uns):
            j2 = (j+1) % uns
            ubm.faces.new((urings[i][j], urings[i][j2], urings[i+1][j2], urings[i+1][j]))
    if uapex:
        for j in range(uns):
            ubm.faces.new((urings[-1][j], urings[-1][(j+1) % uns], uapex))
    ubm.faces.new(list(reversed(urings[0])))
    urn = obj_from_bmesh("cock_urn", ubm); add_subsurf(urn, vp=1, rnd=2); cparts.append(urn)
    # thumb spur: small flat arm sweeping up-and-back to a ball tip
    spur_pts = [(-0.1175, 0.0445, 0.0026), (-0.1235, 0.0530, 0.0021), (-0.1283, 0.0602, 0.0017)]
    spur = build_ribbon_xz("cock_spur", spur_pts, cy, 0.0046); add_bevel(spur, width=0.0009, segments=2)
    cparts.append(spur)
    cparts.append(add_sphere("cock_spball", r=0.0026, loc=(-0.1295, cy, 0.0622), segs=14, rings=8))
    cock = join_objects("Cock", cparts); add_bevel(cock, width=0.0006, segments=1)
...
GRIP_HW = 0.0176                      # grip half-thickness (Y) — widened to meet the side tangs (owner: no gap)
...
def build_grip():
    ...
    # big bevel rolls the broad front/back faces into a ROUND, oval handle section
    # (owner: 'zakulatit' — was reading as a flat rounded slab)
    add_bevel(grip, width=0.0125, segments=6)
...
def build_triggerguard():
    ...
    # BOXY rectangular guard (owner: was too elongated/deep) — vertical sides, a
    # FLAT bottom, rounded corners (not a square), more compact.
    # Moved BACK so the guard starts at the MIDDLE of the round breech collar
    # (x=-0.091) and a little SHORTER in height (owner).
    g_pts = [(-0.091, 0.0, -0.016),   # front finial (at collar middle)
             (-0.094, 0.0, -0.031),   # front side (vertical)
             (-0.098, 0.0, -0.041),   # front bottom corner
             (-0.107, 0.0, -0.044),   # flat bottom (front)
             (-0.124, 0.0, -0.044),   # flat bottom (rear)
             (-0.132, 0.0, -0.040),   # rear bottom corner
             (-0.136, 0.0, -0.030),   # rear side (vertical)
             (-0.141, 0.0, -0.022)]   # rear finial (onto grip belly)
    g_taper = [(-1, 0.9), (-0.5, 1.1), (0.5, 1.1), (1, 0.95)]
    guard = build_curve_tube("TriggerGuard", g_pts, bevel=0.0024, taper=g_taper)
    f1 = add_sphere("tg_fin1", r=0.0034, loc=(-0.090, 0, -0.014)); f1.scale=(1,1,0.8)
    f2 = add_sphere("tg_fin2", r=0.0036, loc=(-0.142, 0, -0.021)); f2.scale=(1,1,0.85)
    guard = join_objects("TriggerGuard", [guard, f1, f2]); add_bevel(guard, width=0.0005, segments=1)
    return guard

def build_trigger():
    """Graceful curved flintlock trigger blade: hangs from the breech, sweeps down
    and curls FORWARD (toward the muzzle) to a pointed tip — owner sketch + Denix
    ref img 1. Tapered (fuller at the pivot, fine at the tip), slightly flattened."""
    # centred in the middle of the guard "trough" (owner)
    t_pts = [(-0.1155, 0.0, -0.0185), (-0.1163, 0.0, -0.0260),
             (-0.1155, 0.0, -0.0325), (-0.1134, 0.0, -0.0370)]
    t_taper = [(-1, 1.15), (-0.1, 1.0), (0.6, 0.72), (1, 0.45)]
    trig = build_curve_tube("Trigger", t_pts, bevel=0.0018, taper=t_taper)
    trig.scale = (1.0, 0.62, 1.0)            # flatten to a blade
    add_bevel(trig, width=0.0004, segments=1)
    return trig

def build_tang():
    """Flat breech TANG side-plate lying flat against the -Y (lock) SIDE of the
    breech/wrist — runs from the breech back toward the grip, tapering to a point,
    with a tang screw on its face (owner: on the side, not on top). Static part."""
    SY = -0.0168                          # -Y side surface (lock side, with the cock)
    # bigger teardrop lockplate face (x, z): fat rounded end at the breech (cock
    # screw), tapering to a point toward the grip (owner: 'trochu větší').
    # stretched ~20% toward the grip (owner): front stays at the breech, rear point
    # extended back from x=-0.135 to -0.147.
    poly = [(-0.0760, 0.0008), (-0.0772, 0.0135), (-0.0868, 0.0155),
            (-0.1036, 0.0144), (-0.1264, 0.0080), (-0.1468, -0.0028),
            (-0.1264, -0.0002), (-0.1048, 0.0016), (-0.0868, 0.0008)]
    tang = extrude_xz("BreechTang", poly, SY - 0.0028, 0.0032)   # thin plate, proud on -Y
    add_bevel(tang, width=0.0009, segments=2)
    # tang/cock screw on the outer face, in the fat end (axis = Y, head facing -Y)
    screw = add_cylinder("tang_screw", r=0.0022, depth=0.0032, loc=(-0.0905, SY - 0.0036, 0.0058),
                         verts=16, rot=(radians(90), 0, 0))
    add_bevel(screw, width=0.0004, segments=1)
    t = join_objects("BreechTang", [tang, screw])
    add_mirror(t, y=True)                 # duplicate onto the +Y side -> 2 tangs (owner)
    return t
...
def main():
    ...
    cluster, bores, collar = build_barrel_cluster()
    assign(cluster, M_BARREL); assign(bores, M_BORE); assign(collar, M_GOLD)
    ...
    # the breech collar is a STATIC sleeve: parented to the root, NOT the pivot,
    # so the barrels revolve inside it (owner: 'tohle bude pevná část').
    parent_to(collar, root)
    built += [cluster, bores, collar]

    # recv = build_breech_receiver(); assign(recv, M_GOLD); built.append(recv)  # owner: removed breechblock
    ...
    tang = build_tang(); assign(tang, M_GOLD); built.append(tang)   # top breech tang (static)
