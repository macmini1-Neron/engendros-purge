/* =====================================================================
   TOLO  –  enemy plyšák (Engendros)
   Vlož níže uvedený `case 'tolo':` do switch(def.shape) ve funkci
...
case 'tolo': {
  ...
  // ── SMYČKA na temeni – otevřená SPIRÁLA/háček (černá látka) ──
  {
    const cx = 0, cy = 0.752;
    ...
    for (let k = 0; k <= M; k++) {
      ...
      const r = 0.072 - 0.038 * f;              // poloměr klesá → spirála
  ...
  // ── TERČÍK – ZAPEČENÝ do bříška (potisk plyše: sférické pásy na povrchu) ──
  {
    const tr = BODY_R + 0.002;
    const ring = new THREE.SphereGeometry(tr, 28, 48, 0, Math.PI*2, 0.362, 0.210);
    b.geo(ring, 0, BODY_Y, 0, cRed, { rx: -Math.PI/2 }); ring.dispose();   // vnější prstenec
    const dot = new THREE.SphereGeometry(tr, 28, 24, 0, Math.PI*2, 0, 0.1885);
    b.geo(dot, 0, BODY_Y, 0, cRed, { rx: -Math.PI/2 }); dot.dispose();     // střed
    // (mezikruží = barva těla)
  }
  ...
  const EY = 0.40;
  // knoflík (+X) – nakloněný do plochy hlavy podle normály
  { const ex = 0.135, n = headNorm(ex, EY), p = headSurf(ex, EY);
    ...
    q = at(0.024);                                   // X-steh nití – bílý jako tělo
    b.box(0.058, 0.010, 0.010, q[0], q[1], q[2], cHead, { ry:  0.78, align: n });
    b.box(0.058, 0.010, 0.010, q[0], q[1], q[2], cHead, { ry: -0.78, align: n });
  }
  // korálek (−X) – malá lesklá koule + souvislá závorka ( kolem poloviny oka
  { const ex = -0.135, n = headNorm(ex, EY), p = headSurf(ex, EY);
