...
test('terrainHeightAt adds the deform offset (crater lowers the ground)', () => {
  ...
  df.add({ x, z, r: 4, depth: 0.8, lip: 0 });   // within the shallow MAX_DIG cap
  assert.ok(Math.abs(t.terrainHeightAt(x, z) - (before - 0.8)) < 1e-9, 'centre dropped by depth');
});
...
test('flat profile still digs (base 0 + deform)', () => {
  ...
  t.deformField.add({ x: 5, z: 5, r: 3, depth: 0.8, lip: 0 });
  assert.ok(Math.abs(t.terrainHeightAt(5, 5) - (-0.8)) < 1e-9);
});
