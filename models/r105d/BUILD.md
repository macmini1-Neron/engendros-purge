...
- Mounted on the **back** with the **harness side (back-pad + webbing straps) sitting AGAINST the plush's back** and the radio **case facing out** (`R105D_YAW=π/2`) + telescopic antenna up — the real worn-pack look. Tunables `R105D_SCALE=2.0 / R105D_Y=0.36 / R105D_Z=-0.44 / R105D_YAW=π/2 / R105D_TILT=-π/9` (mesh-local; def.scale rides them; TILT = the group's `rotation.x`). Owner-tuned over many rounds (the harness is on the model's -X side, the panel on +Z, so they're 90° apart — every yaw was eyeballed): rotation cycled π → π/2 → π → 3π/2 and finally **π/2 so the straps press against the back and the case faces out** (owner: "straps belong against the back, it was 180° reversed"); dropped low on the body (Y 0.85→0.36); then **pulled off the back (z -0.34→-0.44) and pitched −20° backward** (`R105D_TILT`) so it hangs/leans off the lower back like a slung pack instead of sitting flush. No emissive body-glow (removed — read as a night green-light bug). Verified from behind against a clean backdrop. ⚠️ Screenshot-only gotcha: a courier lifted far from the player is draw-distance **culled** (`mesh.visible=false`) — force-show it to render; in-game it's near the player so it's never culled.
- **Bent antenna:** the AŠ-1,5 whip (antSeg1–4 + collars + tip in `spec.json`) is curved along a gentle arc toward the model's +X (progressive `at.x` offset + `rot:[0,0,-3…-23]°`), keeping the brass base flange/nut straight. At the courier's `YAW=π/2` mount that +X arc maps to mob **−Z = leaning backward off the head**, so the whip no longer spears through the plush skull (owner: "bend it naturally so it's not stuck in the head"). The arc is authored in MODEL space for this specific yaw — if the mount yaw changes, recheck the lean direction.
