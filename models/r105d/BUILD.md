...
- Mounted on the **back** with the **harness side (back-pad + webbing straps + buckle) facing OUT the back** (`R105D_YAW=3π/2`) + telescopic antenna up — the worn-pack "spot the courier" tell. Tunables `R105D_SCALE=2.0 / R105D_Y=0.42 / R105D_Z=-0.34 / R105D_YAW=3π/2` (mesh-local; def.scale rides them). **The radio is SET INTO the round back** (z=-0.34) so it sits flush, not cantilevered. Owner-tuned over several rounds: z=-0.55 stuck out → embedded to -0.34; rotation went through π → π/2 → π and finally **3π/2 so the straps show on the back** (owner: "the straps must be on the side where the mob has its back"); dropped low on the body (Y 0.85→0.65→0.52→0.42). No emissive body-glow (removed — read as a night green-light bug). Verified from behind against a clean backdrop. ⚠️ Screenshot-only gotcha: a courier lifted far from the player is draw-distance **culled** (`mesh.visible=false`) — force-show it to render; in-game it's near the player so it's never culled.
