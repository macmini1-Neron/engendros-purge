...
- Mounted on the **back** with the X-cross panel + telescopic antenna facing **outward** (the antenna clears the big plush head — the "spot the courier" tell). Tunables `R105D_SCALE=2.0 / R105D_Y=0.85 / R105D_Z=-0.34 / R105D_YAW=π` (mesh-local; def.scale rides them). **The radio is SET INTO the round back** (z=-0.34, not floated behind) so the panel sits flush/proud instead of cantilevering out — the owner flagged the first pass (z=-0.55) as sticking out wrong; this seats it against the body. Verified flush from back-3/4 + side + front.
