...
- Mounted on the **back** with the **harness side (back-pad + webbing straps) sitting AGAINST the plush's back** and the radio **case facing out** (`R105D_YAW=π/2`) + telescopic antenna up — the real worn-pack look. Tunables `R105D_SCALE=2.0 / R105D_Y=0.42 / R105D_Z=-0.34 / R105D_YAW=π/2` (mesh-local; def.scale rides them). **The radio is SET INTO the round back** (z=-0.34) so it sits flush, not cantilevered. Owner-tuned over many rounds (the harness is on the model's -X side, the panel on +Z, so they're 90° apart — every yaw was eyeballed): z=-0.55 stuck out → embedded to -0.34; rotation cycled π → π/2 → π → 3π/2 and finally **π/2 so the straps press against the back and the case faces out** (owner: "straps belong against the back, it was 180° reversed"); dropped low on the body (Y 0.85→0.42). No emissive body-glow (removed — read as a night green-light bug). Verified from behind against a clean backdrop. ⚠️ Screenshot-only gotcha: a courier lifted far from the player is draw-distance **culled** (`mesh.visible=false`) — force-show it to render; in-game it's near the player so it's never culled.
