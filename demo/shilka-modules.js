...
export const COMMANDER_CONTROLS = [
  ...
  { ru:'НАПРЯЖ. ФАЗ', cz:'volba fází',             n:10, n70:34, kind:'selector', px:{x:50,y:95}, set:'phaseSel', opts:['1','2-2','3-3','Т'] },
  { ru:'ВОЛЬТМЕТР DC', cz:'27/55 V',               n:9,  n70:9,  kind:'gauge', px:{x:82,y:77}, read:(s)=>s.batteryVolts.toFixed(0)+'V' },
  { ru:'ВОЛЬТМЕТР AC', cz:'220/400',               n:8,  n70:8,  kind:'gauge', px:{x:82,y:92}, read:(s,b)=>b.ac220?'220':'0' },
  // — okruh střelby —
  { ru:'ОГРАНИЧЕНИЕ УГЛОВ', cz:'min. náměr °',     n:17, n70:19, kind:'selector', px:{x:12,y:6}, set:'angleLimit', opts:[0,5,10,15,20,25,30,35,40] },
  { ru:'ВОЗБУЖДЕНИЕ', cz:'buzení gen.',            kind:'lamp', px:{x:33,y:5}, col:'amber', read:(s)=>s.generatorOnline },
  { ru:'ШУНТ-СРП', cz:'СРП ↔ ШУНТ',                n:11, n70:18, kind:'toggle', px:{x:32,y:15}, set:'shuntSrp' },
  { ru:'АВАРИЙНАЯ СТРЕЛЬБА', cz:'havarijní palba', n:31, n70:17, kind:'toggle', px:{x:14,y:23}, set:'avariynaya', col:'red' },
  { ru:'СТРЕЛЬБА ВЕРХ.', cz:'horní pár',           n:5,  n70:14, kind:'toggle', px:{x:50,y:21}, set:'bankUpper' },
  { ru:'СТРЕЛЬБА НИЖ.', cz:'dolní pár',            n:5,  n70:15, kind:'toggle', px:{x:50,y:31}, set:'bankLower' },
  { ru:'ЦЕПЬ СТРЕЛЬБЫ', cz:'palebný okruh',        n:26, n70:11, kind:'toggle', px:{x:13,y:54}, set:'tsepFire' },
  { ru:'ЛЮК ОТКРЫТ', cz:'poklop otevřen',          n:19, n70:19, kind:'lamp', px:{x:37,y:66}, col:'red', read:(s)=>!s.hatchClosed },
  // — zbraně / munice (4 automaty: ВЕРХ.ЛЕВ/ПРАВ vnitřní pár, НИЖН.ЛЕВ/ПРАВ vnější) —
  { ru:'ПЕРЕЗАРЯДКА ВЛ', cz:'pneu přebití', kind:'button', px:{x:33,y:22} },
  { ru:'ПЕРЕЗАРЯДКА ВП', cz:'pneu přebití', kind:'button', px:{x:67,y:22} },
  { ru:'ПЕРЕЗАРЯДКА НЛ', cz:'pneu přebití', kind:'button', px:{x:11,y:28} },
  { ru:'ПЕРЕЗАРЯДКА НП', cz:'pneu přebití', kind:'button', px:{x:86,y:26} },
  { ru:'ЗАРЯЖЕНО ВЛ', cz:'nabito', kind:'lamp', px:{x:33,y:30}, col:'red', read:(s)=>s.zarVL },
  { ru:'ЗАРЯЖЕНО ВП', cz:'nabito', kind:'lamp', px:{x:67,y:30}, col:'red', read:(s)=>s.zarVP },
  { ru:'ЗАРЯЖЕНО НЛ', cz:'nabito', kind:'lamp', px:{x:11,y:36}, col:'red', read:(s)=>s.zarNL },
  { ru:'ЗАРЯЖЕНО НП', cz:'nabito', kind:'lamp', px:{x:86,y:37}, col:'red', read:(s)=>s.zarNP },
  { ru:'ОСТАТОК ВЛ', cz:'zbytek', kind:'counter', px:{x:33,y:38}, read:(s)=>String(s.ammoVL).padStart(3,'0') },
  { ru:'ОСТАТОК ВП', cz:'zbytek', kind:'counter', px:{x:67,y:38}, read:(s)=>String(s.ammoVP).padStart(3,'0') },
  { ru:'ОСТАТОК НЛ', cz:'zbytek', kind:'counter', px:{x:11,y:43}, read:(s)=>String(s.ammoNL).padStart(3,'0') },
  { ru:'ОСТАТОК НП', cz:'zbytek', kind:'counter', px:{x:86,y:44}, read:(s)=>String(s.ammoNP).padStart(3,'0') },
  { ru:'РОСА', cz:'hašení ППО', kind:'button', px:{x:85,y:19}, col:'red' },
  // — chlazení / hydro —
  { ru:'УРОВЕНЬ ОЖ', cz:'hladina chladiva', kind:'lamp', px:{x:32,y:53}, col:'red', read:(s)=>s.lowCoolant },
  { ru:'ОХЛАЖДЕНИЕ', cz:'chlazení běží', n:24, n70:123, kind:'lamp', px:{x:48,y:53}, col:'cool', read:(s)=>s.cooling },
  { ru:'ГИДРОПРИВОД ВКЛ', cz:'pohon on', n:16, n70:25, kind:'button', px:{x:63,y:52}, set:'hydraulicOn' },
  { ru:'ГИДРОПРИВОД ВЫК', cz:'pohon off', n:14, n70:27, kind:'button', px:{x:85,y:52}, clear:'hydraulicOn', col:'red' },
  { ru:'54В / 27В', cz:'rozsah voltmetru', n:13, n70:7, kind:'toggle', px:{x:50,y:61}, set:'dcRange' },
  // — ГАГ gyro —
  { ru:'ГАГ', cz:'gyro master', n:27, n70:35, kind:'toggle', px:{x:46,y:89}, set:'gagOn' },
  { ru:'ОТСТОПОРЕНО', cz:'ready', kind:'lamp', px:{x:37,y:75}, col:'green', read:(s)=>s.gagReady },
  { ru:'КОНТРОЛЬ', cz:'self-test', n:33, n70:31, kind:'button', px:{x:55,y:75} },
  { ru:'ЗАСТОПОРЕНО', cz:'náběh ~3 min', kind:'lamp', px:{x:37,y:83}, col:'red', read:(s)=>s.gagPhase==='spinup' },
  { ru:'НЕЙТРАЛЬНО', cz:'neutrál', kind:'lamp', px:{x:55,y:83}, col:'red', read:(s)=>s.gagPhase==='fault' },
];
