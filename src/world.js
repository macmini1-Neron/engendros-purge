...
export class World {
  ...
  _buildSteppe() {
    ...
    for (let i = 0; i < 24; i++) { // boulders on open ground (cover + collision sanity)
      ...
      if (x > -196 && x < -104 && z > 36 && z < 124) continue; // keep the kolkhoz «Красный степной» yard clear
    ...
    buildKolkhoz(this, -150, 70); // kolkhoz «Красный степной» + crashed Su-24 POI, NW open steppe (see kolkhoz.js)
    buildSecretBunker(this, 170, 15); // «Объект 1180» secret command bunker POI, remote E steppe (see bunker.js)
    this.lootSpots.push(new THREE.Vector3(-132, 0, 60), new THREE.Vector3(-150, 0, 86)); // farm yard + by the wreck
  }
