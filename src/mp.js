...
export class MP {
  ...
  _wireNet() {
    ...
    n.on('boom', (d, from) => { if (!this.isHost || !d) return; g.explode(new THREE.Vector3(d.p[0], d.p[1], d.p[2]), { radius: d.r, dmg: d.d, enemyDmg: d.ed, source: d.s, harmEnemies: !!d.he, harmPlayers: !!d.hp, clearLoot: !!d.cl, destroy: !!d.ds, isRocket: !!d.rk, visual: false, net: false }); }); // client thrower's explosion → host authoritatively applies enemy AoE + player FF + item clearing + destruction (visual already shown via the 'proj' ghost)
