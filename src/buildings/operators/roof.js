...
export function gableRoof(b, a, ctx) {
  ...
  b.prism(w + 2 * ov, a.rise, d + 2 * ov, 0, ctx.topY + a.rise / 2, 0, { mat: ctx.mat ?? ctx.materials?.roof, axis, collide: false });
}
...
export function hipRoof(b, a, ctx) {
  ...
  if (axis === 'x') {
    b.prism(midLen, a.rise, D, 0, y + a.rise / 2, 0, { mat, axis: 'x', collide: false });
    b.wedge(hipRun, a.rise, D, -(midLen / 2 + hipRun / 2), y + a.rise / 2, 0, { mat, axis: 'z', hi: 'E', collide: false });
    b.wedge(hipRun, a.rise, D, +(midLen / 2 + hipRun / 2), y + a.rise / 2, 0, { mat, axis: 'z', hi: 'W', collide: false });
  } else {
    b.prism(W, a.rise, midLen, 0, y + a.rise / 2, 0, { mat, axis: 'z', collide: false });
    b.wedge(W, a.rise, hipRun, 0, y + a.rise / 2, -(midLen / 2 + hipRun / 2), { mat, axis: 'x', hi: 'N', collide: false });
    b.wedge(W, a.rise, hipRun, 0, y + a.rise / 2, +(midLen / 2 + hipRun / 2), { mat, axis: 'x', hi: 'S', collide: false });
  }
}
...
export function sawtoothRoof(b, a, ctx) {
  ...
  for (let i = 0; i < a.teeth; i++) {
    ...
    b.wedge(w, a.rise, depth, 0, y + a.rise / 2, zc, { mat, axis: 'x', hi: 'N', collide: false });
    if (a.glazed) {
...
// mirrors shellBox: N/S full width, E/W between them).
